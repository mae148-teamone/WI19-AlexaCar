...
class JoystickController(object):
    ...
    def Determine_Command(self, line):
        ...
        if len(line) == 1:
            ...
            if status == "GPIO.HIGH\n" or status =="GPIO.HIGH":
                ...
                self.emergency_stop()
                print("stopping car")
            elif status == "DRIVEMODE1\n" or status == "DRIVEMODE1":
                self.drive(5)
            ...
            intent = line[0]
            parameter = line[1]
            if intent == "DRIVEMODE1\n" or intent == "DRIVEMODE1":
                self.drive(int(parameter))
            elif intent == "ORIENT\n" or intent == "ORIENT":
                print("orienting")
                #self.set_steering(-1)
                #self.set_throttle(-0.35)
                self.orient(parameter)
            elif intent == "MODE\n" or intent == "MODE":
                self.mode = parameter
                #self.switch_mode(parameter)
            elif intent == "ERASERECORDS\n" or intent == "ERASERECORDS":
                self.erase_records(parameter)
            elif intent == "THROTTLE\n" or intent == "THROTTLE":
                print(parameter)
                self.set_throttle(int(parameter)/100)
            elif intent == "RECORDING\n" or intent == "RECORDING":
                self.set_recording(parameter)
    ...
    def erase_records(self,number):
        if self.tub is not None:
            if number == "ALL":
                number = 10000
            number = int(number)
            try:
                self.tub.erase_last_n_records(number)
                print('erased last %d records.' % number)                
            except:
                print('failed to erase')
    def drive(self, duration):
        #time.sleep(time) not sure
        ...
        while current - initial_time < duration-0.5:
            self.mode = "local"
            #self.set = 1.0
            current = time.time()
        #self.driving = False
    ...
    def orient(self, direction):
        ...
        current = time.time()
        self.set_throttle(-0.35)
        while current - initial_time < 4.5:
            current = time.time()
        self.set_steering(0)
        #self.set_throttle(0)
        self.emergency_stop()
    ...
    def update(self):
        ...
        while self.running and self.js is None and not self.init_js():
            f = open('/home/pi/d2t/alexa_output', 'r+', os.O_NONBLOCK)
           # print("f is")
           # print(f)
            line = f.readline()
            if line:
                print(line)
                self.Determine_Command(line)
